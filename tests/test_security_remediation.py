...
def test_ffi_signature_length_validation():
    ...
    math_lib = load_dylib(
        "test_ffi_val",
        signatures={"add_numbers": {"args": ["i32", "i32"], "ret": "i32"}},
    )
...
def test_symbol_cache_pollution_prevention():
    ...
    lib = load_dylib(
        "cache_pollute_lib", signatures={"add_one": {"args": ["i32"], "ret": "i32"}}
    )
    ...
    with pytest.raises(RuntimeError):
        ...
        task_id = submit_dylib_task("cache_pollute_lib", "add_one", b"hello")
        h = TaskHandle(task_id)
        h.result()
...
def test_task_group_cancellation_filtering():
    ...
    async def run():
        ...
        assert any(
            isinstance(e, RuntimeError) and "Root error" in str(e) for e in exceptions
        )
...
def test_unregister_dylib():
    ...
    lib = load_dylib(
        "temp_unregister_lib",
        signatures={"multiply_numbers": {"args": ["i32", "i32"], "ret": "i32"}},
    )
