...
| `PYROXIDE_WASM_MEMORY_LIMIT_BYTES` | Maximum memory allowed for a single WASM instance in bytes. | None (defaults to `104857600` / 100MB) |
| `PYROXIDE_WASM_TIMEOUT_MS` | Timeout for WASM execution in milliseconds. | None (defaults to `1000` / 1s) |
